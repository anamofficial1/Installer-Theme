...
echo -e "${BLUE}"
echo "=================================================="
echo "           PROTECT BY ANAM INSTALLER"
echo "           Created by Anam official"
echo "=================================================="
echo -e "${NC}"
...
class UserController extends Controller
{
    ...
    public function delete(Request $request, User $user): RedirectResponse
    {
        ...
        if ($request->user()->id !== 1) {
            throw new DisplayException("❌ Hanya admin ID 1 yang dapat menghapus user lain! © ᴘʀᴏᴛᴇᴄᴛ ʙʏ ᴀɴᴀᴍ - ᴘʀᴏᴛᴇᴄᴛ ᴠ1.0.0");
        }
        // ============================================
    ...
    public function update(UserFormRequest $request, User $user): RedirectResponse
    {
        ...
        foreach ($restrictedFields as $field) {
            if ($request->filled($field) && $request->user()->id !== 1) {
                throw new DisplayException("⚠️ Data hanya bisa diubah oleh admin ID 1. © ᴘʀᴏᴛᴇᴄᴛ ʙʏ ᴀɴᴀᴍ - ᴘʀᴏᴛᴇᴄᴛ ᴠ1.0.0");
            }
        }

        // Cegah turunkan level admin ke user biasa
        if ($user->root_admin && $request->user()->id !== 1) {
            throw new DisplayException("🚫 Tidak dapat menurunkan hak admin pengguna ini. Hanya ID 1 yang memiliki izin. © ᴘʀᴏᴛᴇᴄᴛ ʙʏ ᴀɴᴀᴍ - ᴘʀᴏᴛᴇᴄᴛ ᴠ1.0.0");
        }
        // ====================================================
...
class NodeController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin ID 1 yang dapat membuka menu Nodes. © ᴘʀᴏᴛᴇᴄᴛ ʙʏ ᴀɴᴀᴍ - ᴘʀᴏᴛᴇᴄᴛ ᴠ1.0.0');
        }
        // ======================================================
...
class NestController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak! Hanya admin utama (ID 1) yang bisa membuka menu Nests. © ᴘʀᴏᴛᴇᴄᴛ ʙʏ ᴀɴᴀᴍ - ᴘʀᴏᴛᴇᴄᴛ ᴠ1.0.0');
        }
...
class IndexController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $user = Auth::user();
        if (!$user || $user->id !== 1) {
            abort(403, 'ᴀɴᴀᴍ ᴘʀᴏᴛᴇᴄᴛ - ᴀᴋꜱᴇꜱ ᴅɪᴛᴏʟᴀᴋ ❌');
        }
...
echo -e "${BLUE}Starting Protect by Anam installation...${NC}"
echo ""
...
echo ""
echo -e "${GREEN}==================================================${NC}"
echo -e "${GREEN}    PROTECT BY ANAM INSTALLATION COMPLETE!${NC}"
echo -e "${GREEN}            Created by Anam official${NC}"
echo -e "${GREEN}==================================================${NC}"
echo ""
...
echo ""
echo -e "${BLUE}Your Pterodactyl panel is now protected by Anam!${NC}"
echo ""
